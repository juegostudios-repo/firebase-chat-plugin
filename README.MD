...
### If you were using older version of this package, before updating you need to clear firebase database.
...
This method is used to update both user and group display picture. To update group display picture pass the groupID and channelType = 'group'. To update user display picture pass the channelType = 'one2one'.
```
...
### Sending message
```
app.sendMessage(otherUserId, message, channelType, messageType /*optional*/, fileKey /*optional*/)
.then(res => {
  console.log(res);
})
.catch(err => console.log(err));
...
### Sending Image
```
app.sendImage(imageUrl, otherUserId, channelType , messageType)
.then(res => 
    console.log(res),
  err => console.log(err));
```
...
```
app.getMessageList(otherUserId, channelType)
.subscribe(message => {
    console.log(message);
  }, 
  err => console.log(err));
